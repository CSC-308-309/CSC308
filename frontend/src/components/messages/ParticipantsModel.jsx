export default function ParticipantsModel({ isOpen, onClose, participants, title }) {
              ...
              {participants.map((p) => (
                <div key={p.username} className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center">
                    {p.avatar ? (
                      <img src={p.avatar} alt={`${p.username} avatar`} className="w-full h-full object-cover" />
                    ) : (
                      <span className="text-gray-600 font-semibold">{(p.name || p.username)?.[0]}</span>
                    )}
                  </div>

                  <div className="min-w-0">
                    <div className="font-medium text-gray-800 truncate">{p.name || p.username}</div>
